# BRUTEFORCE ATTACK BY ICONIC_VENOM & WOTAX
# TO RUN THIS PYTHON PROGRAM FIRST INSTALL THE MODULES MENTION IN THE REQUIREMENTS.TXT
# USE pip install requests pycryptodome protobuf-decoder colorama urllib3 cmd to install lib
# TO START USE "source venv/bin/activate" cmd then python generator.py OR python3 generator.py According to system
#it will start to make new id's but will do fake attacks 3 times in a row after each successful creations to avoid ip ban
import hmac
import hashlib
import requests
import string
...
def generate_random_name():
    ...
    name = "VIGNX" + "".join(random.choice(characters) for _ in range(6)).upper()
    return name
...
def GET_PAYLOAD_BY_DATA(
    ...
    try:
        ...
        if len(parts) < 2:
            print("Invalid JWT token format:", JWT_TOKEN)
            return None
        token_payload_base64 = parts[1]
...
# UID'S AND PASSWORD TOKEN WOULD BE ADDED I VIGNX GEN.TXT
...
║                                           ★彡[ᴠɪɢɴᴇꜱʜ]彡★    ║
╚══════════════════════════════════════════════════════════════╝
  ░    ░  ░         ░    ░  ░   ░           ░  ░             ░ ░     ░     \n                                           {Fore.RED}@{Fore.RESET}Dev: VIGNX\n"""
)


region = input(
    f"{bold}{red}[{lg}+{red}]{red} Choose Region (ME , IND , VN , BR ...) : {Fore.RESET}"
).upper()


for i in range(50000):
    with open("BY VIGNX gen.txt", "a") as file:
        try:
            r = create_acc(region)
            response = r["response"]
